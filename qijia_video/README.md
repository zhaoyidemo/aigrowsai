...
Seedream 使用同步图片接口时将连接、写入和生成读取分开处理：生成读取最多等待 600 秒；仅在连接尚未建立时安全重连一次。请求一旦可能已被方舟接收，系统绝不自动重提，并把该次调用标为“可能计费、待对账”；每张成功图片都会先持久化再继续，任务恢复只补尚未保存的图片。

H3 Provider Adapter 的方法正文只作为内部编译规则，不再原样塞进 Seedream 或 Seedance 的最终提示词。新图片提示词按可见事件优先做分段压缩并限制在 4000 字符内；候选领域契约统一容纳 6000 字符以兼容历史快照，且所有本地字段都在付费提交前完成预校验。
