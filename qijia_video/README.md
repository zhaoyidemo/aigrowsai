...
创建页以自然语言主输入为第一动作，只保留视觉风格这个常用选择，并用同一场景的真实样片进行比较。页面通过 `/capabilities.runtime_models` 展示后端实际加载的脚本、导演、图片、视频与配音模型及精确 ID；这些信息只读，不形成第二套前端配置。内部角色和中间规划不作为创建控件，冻结版本仍进入发布包审计快照。
