...
- 六组家庭教育方向分别读取 TikHub 抖音指数视频库的“低粉爆款”与“高点赞率”精选标签；低粉查询固定为近 3 天，高热补充固定为近 7 天，均筛选母婴垂类和 15–60 秒视频。接口明确返回发布时间、播放、点赞、评论、分享与作者粉丝等核心数据：<https://docs.tikhub.io/443673045e0>
- TikHub 提供“低粉爆款”标签和独立榜单，但没有公开内部判定公式。系统不会把平台标签直接当结论，而是采用齐家自己的严格复核：低粉爆款必须发布不超过 72 小时，同时满足粉丝不超过 5 万、播放不少于 50 万、播粉比不低于 20、赞播比不低于 5%、深度互动率不低于 0.8%；高热爆款必须发布不超过 7 天，同时满足播放不少于 100 万、赞播比不低于 8%、深度互动率不低于 0.8%。深度互动率按 `(评论 + 分享 + 收藏) / 播放` 计算：<https://docs.tikhub.io/252393854e0>
...
TikHub 文档的示例响应没有提供稳定的业务 `data` 样例，因此适配器采用保守归一化：无法识别有效日期会立即停止；必须至少有 10 条通过复核的时效爆款视频且其中至少 5 条为 72 小时内低粉爆款，否则不会调用编辑模型；每个候选必须由至少 2 条独立爆款视频共同验证且包含低粉爆款，五个候选合计引用至少 8 条不同爆款视频和 5 条不同低粉爆款，排名前三位还必须分别引用不同的低粉爆款。不保存庞大的原始响应，只保存候选可复核所需的证据快照和请求 ID。
