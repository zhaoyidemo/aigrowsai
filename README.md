...
成片确认前的故事板采用混合制作：每个章节都可以继续使用 AI 素材，也可以上传 JPG、PNG、WebP 图片（最大 20 MB）或 MP4、MOV、WebM 视频（最大 200 MB）。上传图片由 Remotion 统一竖屏裁切并添加轻微取景运动；上传视频由 FFmpeg 转为静音 H.264、从开头截取，并在不足章节时长时冻结最后一帧。上传、历史上传版本和 AI 版本都保留在任务中，可随时切换；任何一次处理或质检失败都不会覆盖上一版可确认成片。
