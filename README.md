...
- 本版本只采集抖音作品播放量，明确忽略小红书和视频号，也不采集 APP 下载或注册归因。
...
- 播放量不会自动更新；每次绑定或点击“手动刷新播放量”只发起 1 次 TikHub 请求，操作按钮会提前显示人民币规划成本；成功、明确失败响应和网络结果未知分别按“规划价、¥0、待对账”入账。响应缺少播放量时不会按 0 保存。
...
- 播放回流复用同一批量视频详情端点；`v.douyin.com` 短链首次通过 Web 分享链接接口读取单个作品。官方说明该接口接收 `share_url` 并返回作品数据：<https://docs.tikhub.io/257556744e0>
...
团队效果看板只汇总已手填绑定的抖音作品，首屏聚焦累计播放、累计视频成本、整体 ROI、团队 10 倍目标差距和单条视频 ROI 排行；供应商、阶段、调用明细和计价依据默认折叠在“成本明细与对账”中。时间范围按作品首次绑定时间确定发布批次；手动刷新不会把旧作品移入新批次。作品一旦纳入，播放量使用最新累计快照，成本使用该视频全生命周期累计成本，避免用累计播放除以局部成本造成虚高 ROI。同一抖音作品被误绑定到多个任务时，明细保留全部绑定供排查，团队合计只按最早绑定任务计一次。尚未绑定的已打包视频只计入回流覆盖缺口；选题研究费用不强行分摊到单条视频。

账本覆盖当前全部内容生产收费节点：

- TikHub：选题研究和抖音播放回流都逐次保存成功/失败、端点和请求 ID，按成功请求规划价估算；原始美元金额在报表中固定按 `1 USD = ¥6.7` 换算。
...
真实部署验收至少包括：登录、生成并确认脚本、完整旁白、五张首帧、三段视频、Remotion 成片、发布包下载、手填一条真实抖音作品链接并刷新播放量、核对单视频与团队看板的 10 倍 ROI、排行和 CSV、与供应商账单抽样核对，以及服务重启后的任务与播放快照可见性。
